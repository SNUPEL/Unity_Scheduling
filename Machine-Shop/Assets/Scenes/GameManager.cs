...
public class GameManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        for (int i = 0; i < HousePrefab.transform.childCount; i++)
        {
            houseComponents.Add(HousePrefab.transform.GetChild(i).gameObject);
        ...
        for (int d = 0; d < numBlocks; d++)
        {
            ...
            jobs[d] = new Job(colorData[coloridx], d, houseComponents[1], created, jobdata_list[d]);
            Debug.Log("---------------------------------------------");
    ...
    void Update()
    {
        ...
        {
            if (jobs[i].block.transform.position == positionData[i])
            {
                Instantiate(houseComponents[i]);
            }
        }
        check_termination();
